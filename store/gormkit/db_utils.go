...
func DoGormDBTransaction(db *gorm.DB, fns ...func(db *gorm.DB) gotools.ErrorInfo) gotools.ErrorInfo {
	db = db.Begin()
	...
	err := db.Commit().Error
	if err != nil {
		return gotools.NewError(http.StatusInternalServerError, "transaction commit failed", err)
	}
	return nil
}
