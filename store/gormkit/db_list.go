...
func (g *gormDB) GetCount(t any, listParam *pkgTypes.ListParams) (int64, error) {
	var count int64
	db := g.DB.Model(t)
...
func (g *gormDB) ListWithCount(listParam *pkgTypes.ListParams, t any, listObj any) (int64, error) {
...
func (g *gormDB) ListAssociationsWithCount(listParam *pkgTypes.ListParams, t any, listObj any, items ...string) (int64, error) {
	count, err := g.GetCount(t, listParam)
	if err != nil {
		return 0, eris.Wrap(err, "get count error")
	}
...
// list := make([]Model, 0)
// List(&list)
func (g *gormDB) List(listParam *pkgTypes.ListParams, listObj any) error {
	db := optListDB(g.DB, listParam, listObj)
	return db.Find(listObj).Error
}

func optListDB(db *gorm.DB, listParam *pkgTypes.ListParams, listObj any) *gorm.DB {
...
func (g *gormDB) ListAssociations(listParam *pkgTypes.ListParams, listObj any, items ...string) error {
	db := optListDB(g.DB, listParam, listObj)
	return getAssociations(db, items...).Find(listObj).Error
}

func GenerateDBForQuery(db *gorm.DB, listParam *pkgTypes.ListParams, listObj any) *gorm.DB {
...
func filterQueriesByStructFields(listObj any, queries map[string]string) map[string]string {
	...
	for i := 0; i < typ.NumField(); i++ {
		field := typ.Field(i)
		snake := utils.ConvertToSnakeCase(field.Name)
