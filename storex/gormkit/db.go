...
func (g *gormDB) DoDBTransaction(fns ...func(db database.Database) errorx.ErrorInfo) errorx.ErrorInfo {
...
func (g *gormDB) EnsureExist(t any, id string) errorx.ErrorInfo {
	err := g.Get(t, id)
	if err != nil {
		...
		return errorx.NewError(http.StatusBadRequest, "unexpected error", err)
	}

	return nil
}

func (g *gormDB) EnsureNotExistByName(t any, name string) errorx.ErrorInfo {
	return g.EnsureNotExistByField(t, "name", name)
}

func (g *gormDB) EnsureNotExistByField(t any, key string, value any) errorx.ErrorInfo {
	err := g.GetByField(t, key, value)
	if err == nil {
		return errorx.NewError(http.StatusConflict, "already exists", nil)
	}
	...
	return errorx.NewError(http.StatusInternalServerError, "unexpected error", err)
}
