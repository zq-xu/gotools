...
def process_package(pkg_name: str):
    ...
    EXPORT_typesx = re.compile(r'type\s+([A-Z]\w*)\s*(?:=|\w|struct|interface)')

    file_exports = defaultdict(lambda: {"funcs": [], "typesx": []})
        ...
        if file.endswith(".go") and not file.endswith("_test.go"):
            ...
            if os.path.isfile(path):
                with open(path, "r", encoding="utf-8") as f:
                    ...
                    typesx = EXPORT_typesx.findall(content)
                    ...
                    if typesx:
                        file_exports[file]["typesx"].extend(typesx)
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        has_typesx = any(len(exports["typesx"]) > 0 for exports in file_exports.values())
        if has_typesx:
            f.write("type (\n")
            for file, exports in file_exports.items():
                if exports["typesx"]:
                    f.write(f"    // From {file}\n")
                    for t in exports["typesx"]:
                        f.write(f"    {t} = {pkg_name}.{t}\n\n")
            f.write(")\n")

    total = sum(len(v["funcs"]) + len(v["typesx"]) for v in file_exports.values())
